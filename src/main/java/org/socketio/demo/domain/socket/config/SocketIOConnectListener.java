    ...
    public void OnConnect(SocketIOClient socketIOClient)   {
        ...
        for (String eaCookie : cookies) {
            ...
            if (isValid) {
                ...
                if (authentication != null)socketSessionManager.
                        saveNames_SidAtConnect(socketIOClient.getSessionId(),authentication.getUsername());
                // names_sid[sid] = username 저장
            }
        }
        // 아래부터는 필요없음 이벤트 보내는거임 connect 때에는 이벤트 보낼 필요없음
...
////                 server.getAllClients().stream().filter(client->!client.getSessionId().
////                         equals(target.getSessionId())).forEach(client->client.sendEvent
////                         ("hello", data));
//            }
